...
**并拢改切片陈列（2026-08-20，用户「居中对齐 + 宽度 0.6 倍」）**：带深 26→15.6（间距不动 ⇒ 片间留缝）；排布加 `center`（逐帧把各片折叠体中线对齐到全员均值——纯渲染纵移，`fillSolidVerts` 尾参 offY，形态与物理零动）与 `frame:false`（居中后板/轨对不上挂点，收掉）；分列原样。渲染改两遍（先全员剖面求公共中线再画）。
