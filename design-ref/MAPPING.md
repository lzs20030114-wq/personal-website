...
## 12. 四个项目各进各的详情页（2026-07-29，用户拍板「主页进 1234 项目只有 1 进了自己的详情页，234 也都进一个各自的详情页」）
...
`content.test.ts` 新增 2 项：四个主项目每个都在路由名单里（否则主页卡片指向 404，卡片 href 与 `generateStaticParams` 读的是同一份名单）／非主项目的 draft 不出路由。统计条 191 → **193**。
