...
**并拢改切片陈列（2026-08-20，用户「居中对齐 + 宽度 0.6 倍」）**：带深 26→15.6（间距不动 ⇒ 片间留缝）；排布加 `center`（逐帧把各片折叠体中线对齐到全员均值——纯渲染纵移，`fillSolidVerts` 尾参 offY，形态与物理零动）与 `frame:false`（居中后板/轨对不上挂点，收掉）；分列原样。渲染改两遍（先全员剖面求公共中线再画）。**对位改物理（同日二轮纠偏，用户「上端贴天花对齐，改形状在线上的位置来居中」「间距也 0.6×」）**：渲染纵移方案全回退；lead = 形状在带上的位置（纯平移、每节 2px 与 r 无关），标定得**全员 lead = 54 常数**（中线散布 14.79→1.22px），顶端天然贴天花、板/轨回归；间距 27.5→16.5。端点守门改卡键谱段对象同一；冒烟升级 12 台全员 + 中线散布 ≤2px。**二次标定（同日，用户「还是没齐」）**：判据从「自由段整段中线」改为**凸出体中线**（贴轴缓冲料藏在竖带里、不该算进去），重解整数最优 lead 表，散布 2.95→**1.64px**（量子下界 1.72）；另加尾段配平表使各带总长恒定，修掉帘子下缘 10px 参差。**第三轮**：查明剩余「不齐」是视角的深度错位（12 条同形状实验实证），SolidLayout 加 `home` 机位——并拢默认「侧」（俯仰 0 ⇒ 深度不产生纵向偏移）、分列默认「轴测」。**相机转盘模式（同日）**：OrbitCamera 加 mode/upAxis（详见 CLAUDE.md 装备条目与立体求解器 spec 附注），Lab.04/07/08 启用、Lab.03 与 src/demo 保持轨迹球；守门 +4，统计条 319→**323**。
