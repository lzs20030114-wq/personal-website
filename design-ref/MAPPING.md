...
### 7.9 Lab.04 蒙皮改半透明点阵（2026-07-29，用户拍板「死灰一片，柔和一些」）

**这是对稿的一处有意偏离**（§7.7 刚把「织物蒙皮」按稿补齐，本轮把它的**呈现方式**换掉）：稿内蒙皮是不透明烟灰实体面，实测在暗底上是一整片压过来的灰布——内部机构（十四板三角桁架、曲柄轮、脚槽）全被盖住，五环只剩两端轮廓露在外面。用户否决，改点阵。**几何来源与锚固点一字未改**（仍是相邻环外侧支点等弧长重采样出的直纹带），换的只是这张带子怎么画。

- **画法**：直纹带按 `SKIN_U`(58) × `SKIN_V`(11) 取**格心**采样成点云（`bakeRuledPoints`，纯函数、有单测），逐帧烘焙上传，`drawPointCloud` 以 `gl.POINTS` 画。取格心不取等分端点：`v=0/1` 正落在两侧折线上，相邻两带会在共用的那条环线上叠出双倍密度、还与线稿重影。
- **点色 = 族系配色的横向渐变**：一条带从 A 环色渐变到 B 环色（S1 绿 → S5 紫）。稿里配色只由线承载，蒙皮是中性灰；现在整个身体都带上族系色，这正是「不再死灰」的主因。
- **形体交给不透明度、颜色只轻微压暗**（`rgb×(0.74+0.26λ)`、`α=A×(0.3+0.7λ)`）：暗底上若照实体面那样再乘明暗，背光的点会压成灰渣——实测第一版就是这毛病。
- **点径是世界单位**（`SKIN_DOT_R` 1.75mm，着色器里 `/w` 做透视缩放、clamp 到 1–14px）：缩放时点跟着变大，密度观感恒定；透视开关下近大远小自然成立。
- **深度只测不写 + `blendFuncSeparate`**：点之间不自遮挡（否则绘制序决定谁盖谁、旋转时闪烁），但仍被前方实体正确挡住。canvas 是 premultipliedAlpha 的透明底，alpha 通道必须走 `(ONE, 1−SRC_ALPHA)`，否则叠花底色。
- **绘制序反过来**：实体蒙皮时代是「先铺面、后画线」；半透明必须**后画**才能与已成像的线稿混合，故 render 里点阵放到最后。
- 手感常量 `SKIN_U / SKIN_V / SKIN_DOT_R / SKIN_ALPHA` 全在 `RingsBench.tsx` 顶部，**待用户真机拍板**。

**装备侧仍是加法式扩展**：`gl3d` 新增 `drawPointCloud` + `bakeRuledPoints`（新程序 `POINT_VS/FS`），既有 `drawMesh/drawSkinned/drawDynamicMesh/drawLines/drawDots` 一行未改，GL 状态用完即还原。`drawDynamicMesh` 的 dark/lite 端色（§7.7 那次加的）现在 `RingsBench` 不再用到，但接口保留——`src/demo/*` 仍在用。
