...
原落地段是「不透明平面盖场 → `clipPath` 从满屏收回到 hero」——**裁出来的**，不是飞过去的。现改为真正的共享元素变位缩放，两段接成一条：
...
- **起点取克隆此刻的视觉框**，不重新按视口算——第一段的末帧就是第二段的首帧，接缝为零。没有克隆时（窄视口无面板、或克隆源尺寸不足的兜底路径）才退回按视口 cover 算一帧，hero 自己缩回去，同样不露底。
- **不再需要着陆平面**：起点盒本来就盖满视口（cover ×1.02），hero 自身的渐变底 + `.ground-plane` 兜底，白屏没有落脚点。`.pt-veil` 那条老路**原样保留**给没有 `data-pt-target` 的路由（`/archive` `/lab`）。
...
`MORPH_MS` 760 / `CROSSFADE` 0.55 / `REVEAL_MS` 560 + 140ms 延迟（第一段 `PUSH_MS` 620 不动）——**待用户真机拍板**。真浏览器（CDP，1440×900）实测五条路径：转场落位后残留归零、hero 内联样式复原、无 pageerror；直接访问 / reduced-motion / `/archive`（无落点走老路）/ 回退后二次转场 / 窄视口无克隆兜底，全部干净。统计条 122 → **131**（新增 flip 9 项）。
