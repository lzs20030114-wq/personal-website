...
用户把项目一 2026.03–07 各次会话产出的表格汇成一份附录（约 30 张，带编号 / 来源会话 / 状态标签），原文存 `轮回机器_工作日志表格附录.md`。**十六张挂上站**，落到六条既有条目：03-20（三方向 × 五层「活着」框架）· 04-03（人格参数 / 皮电窗口 / 触碰编码 / 预期结果，四张）· 06-15（五环几何 / 十一个三角形类 / 现行中央驱动 / 最早算错那版，四张）· 07-07（六路输入 / 抓握真值表 / 状态机事件，三张）· 07-09（必买 / 条件购买）· 07-18（加强前后 / 新架构验证）。**条目正文与标签一字未动，只加 `blocks`**；机制零改。池内表格数 7 → **23**，条目数仍 54。

- **上站与否逐表判**，判据是「这张表现在还成立吗」。留在存档不上站的：`待核对`（人格顺序表与参数表的代号冲突；量表组在 07-07 复审里被裁剪过、现行版未确认）、`数据不全`（距离编码只剩 4 条、Fischer 八类只剩 2 类，原文截断）、`已作废`（中央驱动中间版、生理表里被划掉的 HRV 三行）、以及版本冲突汇总这类内部待办。**理由**：站上放一张写着「缺失，待补」或带删除线的表，外部读者只会读成「这就是现状」——存档要完整，站上要成立。
- **略去两列**（各自有据）：五环几何的侧倾角一列——本站 07-17 五环记录已判定它不是装配侧倾、语义待澄清，照抄等于对外发布一个我们自己不认的说法；中央驱动表的第三列 `ratio`——用户原文就标了「定义不明，回原对话确认后再往外发」。两处都在表注里写明略去了什么。
...
- **版式没动**：5 列的人格参数表在 62ch 正文列里靠换行排下，`.log-table__scroll` 的横向滚动兜底未触发，整页无横向溢出（1280px CDP 实测）。若日后表更宽再考虑给 `.log-table table` 加 `min-width`，本轮不动 CSS。180 测试绿、双构建过、23 张表全渲染、无 pageerror。
