...
抖动全在成形期（300–600 步最高 7.2px/步；终态 0.00px）——v7 作者在 GIF 里用帧间指数平滑（0.55/0.45）盖的，台架渲染补同款（SkinBench 帧间 EMA，keep=0.45^(n/20) 按实际步数折算，重播重置；物理零改）。平直度 = boxSquare 加「顶/底面向嘴角→端角连线垂足投影」（只压横向，沿线归拉伸/直化管），面波动 4.7→2.0px。守门加两条断言，仍 305。细节=项目二_皮肤单元lab.md §8。
