...
- **单级成形顺滑（2026-08-30 第二轮，用户「还是不够顺滑：抽一个单独做它的变化过程，做顺滑再推广」）**：在 L5 上逐步量位移破案（新工具 `scripts/skin-dual/process.mjs`）——外箱十键同一步全锁（34.5px/步 = 基线 114 倍）+ 无锁定屈曲翻越（23px）+ 单键触发硬投影（L3 29.6px）。修法 = 引擎两个加法式选项组合（默认关 = v7 逐字）：`lockGap 24` 只圈外箱链（`lockGapChains:[0]`——缝链预锁不能碰，L8 实测绞死）+ `stepClamp 4px`（机制无关限速兜底；裸用会让缝料输掉与箱体合拢的竞速、Δ3.29→9.88）。十级 Δ 与锁定集合逐位复原、过程峰值 34.5→4.0px/步、瞬态环 ≤4、末锁 783–942 全在 950 前。守门 +1（成形顺滑）、连续性阈值按「拉链波」重标；台架/机位/排布零改。统计条 394→395。细节 = 项目二_皮肤单元lab.md §15.12、§16.3 追加第 4 件工具。
