...
export function SkinBench({
  ...
  useEffect(() => {
    ...
    let lastHud = '';
    const terminalInputs = SKIN_UNITS.map((def) => ({ spec: def.spec, opts: skinSiteOpts(def) }));
    const commitTerminal = (states: Awaited<ReturnType<typeof requestSkinTerminal>>['states']): void => {
      units.forEach((v, u) => {
        v.sim.applyTerminalState(states[u]);
      ...
      holdT = 0;
      units.forEach((v) => drawUnit(v));
      const locked = units.reduce((sum, v) => sum + v.sim.locked.length, 0);
      lastHud = `${SKIN.STEPS}|${locked}|held`;
      setHud({
        r: units[units.length - 1].sim.r,
        step: SKIN.STEPS,
        locked,
        phase: 'held',
      });
