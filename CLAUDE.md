...
- **触手 B+C 实现完成（2026-07-10，待真机手感拍板后关闭）**：内核门控解锁（`Bar.stiffness` + Verlet 固定子步动力学，默认关、旧路径逐字节回归锁死，SPEC §1.2/§2.1/§3.6/§4.1/§4.2 修订入档）+ 触手实例（刚性脊柱 14 段 + 软弯曲杆根粗梢细 + 重力/阻尼）+ 台架 `/demo/tentacle.html`（taper 渲染，三台架页互链）。46 项测试绿；真浏览器实测悬垂/甩鞭/摆动衰减正常。**余项：用户真机调参拍板（g/damping/弯曲刚度），拍板后触手关闭。**决策与参数：轮回机器_触手spec.md v0.3。
