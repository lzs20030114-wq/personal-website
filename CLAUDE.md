...
- **立体求解器 3D-M1+M2 完成（2026-07-10，用户当日立项当日开工）**：独立 3D 内核 `solver3d.ts`（与 2D 内核平行、不改其一行）+ 三肌腱 120° 立体触手（GH 机制全量升维，contraction1/2/3 全部可用）+ 台架 `/demo/tentacle3d.html`（**拖拽=轨道相机不动模型**、三肌腱滑块、SVG 正交投影 + 画家排序 + 深度明暗，WebGL 红线不碰）。59 项测试绿；真浏览器验证视角旋转（err 不动）与三腱各自弯向自身方位。已知局限入 spec：GS 扭转偏差（横向漂移 −7~−44px）、画家排序闪序。spec = 轮回机器_立体求解器spec.md v0.1；3D-M3（拾取拖拽/fascia 抗扭）未排。
