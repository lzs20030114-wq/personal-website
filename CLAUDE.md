...
- **立体求解器 3D-M1+M2 完成（2026-07-10，用户当日立项当日开工）**：独立 3D 内核 `solver3d.ts`（与 2D 内核平行、不改其一行）+ 三肌腱 120° 立体触手（GH 机制全量升维，contraction1/2/3 全部可用）+ 台架 `/demo/tentacle3d.html`（**拖拽=轨道相机不动模型**、三肌腱滑块、SVG 正交投影 + 画家排序 + 深度明暗，WebGL 红线不碰）。59 项测试绿；真浏览器验证视角旋转（err 不动）与三腱各自弯向自身方位。**用户实测否决首版（单腱收缩成螺旋+抽搐）→ 3D-M3 提前执行：fascia 双手性抗扭斜杆（横向漂移 38.5→1.2px，平面 C 弯成立）+ 肌肉限速渐变（1.2/s）+ damping 0.992**。spec = 轮回机器_立体求解器spec.md v0.1（§2 处置记录）；3D 拾取拖拽未排。
