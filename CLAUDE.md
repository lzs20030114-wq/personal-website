...
- **M3 实现完成（2026-07-10，待真机手感拍板后关闭）**：① release 阻尼——松手继承甩动末速、指数松弛接回自转（参数可调，τ=0 = 论点实验的硬切对照开关，SPEC §4.2 修订已回填）；② 耦合曲线实时描绘（ring buffer + 断笔）；③ 工程标注/题栏（网格衬底/接地符号/尺寸标注/FIG. 01 题栏实时 θ·ω·残差·模式）。36 项测试全绿；真实浏览器验证：自转 5s 描线 207 段单笔、甩拖松手 ω 1.42→0.90 平滑接回。**余项：用户真机手感拍板（阻尼参数在场调），拍板后 M3 关闭、触手 B+C 解锁。**手感即论点（THESIS_NOTES.md）。
...
- **触手立项（2026-07-10 用户拍板）**：选 B+C（Bar 加 stiffness + Verlet 门控动力学，≈1.5 session），**排 M3 之后**，spec = 轮回机器_触手spec.md v0.2。开工三条件：M3 关闭、用户给 GH 参考录屏（或口头验收标准）、用户在场调参。原「不做」决策改判为「已立项、按队列执行」；动工前不写代码。
