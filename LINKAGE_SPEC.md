...
一句话：在 Grasshopper 里我是 Kangaroo 的**使用者**，这个组件证明我是求解器的**实现者**。它是 CMU MSCD 申请材料中「独立计算产出」的关键一块。

**双重使命（2026-07-07 确认）**：本组件属于「轮回机器」项目——技术信标之外，它同时是「机器由何让人感受到生命感」这一论点的载体，**手感即论点**（机制映射表见 THESIS_NOTES.md）。三种出场形态共用同一引擎：主页 FIG（工程图纸口吻，不解释）、轮回机器项目页（论点先行）、求解器 case study（满篇数学）。因此：代码要能被我本人脱稿讲解，数学要经得起面试追问，运动品质要经得起「它活着吗」的注视。
...
| **M3**（Session 4） | 耦合曲线描绘 + 工程标注/题栏 + release 阻尼接自转（**手感即论点**——生命感的呼吸/恢复线索，见 THESIS_NOTES.md，优先级高于一切抛光） | 松手后平滑接回自转；P 曲线连续描绘、断笔正确；标注就位 |
