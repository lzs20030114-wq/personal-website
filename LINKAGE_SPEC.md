...
- 板朝向符号：`sign( cross(C−B, P−B) )`。温和运动（含整周自转）中恒定；但**暴力拖拽可翻面**——Session 2 用户实测证伪了「永远恒定」：软目标单遍注入可达 0.4×指针距离，一步即可把 P 推过 BC 线（板高仅约 73px），而距离约束不编码手性，投影会收敛到镜像解。物理刚板做不到这件事（两个镜像解在约束流形上不连通），这是迭代刚性特有的数值隧穿。处置见 §5 条 6b。
...
**注意**：目标在可行域外期间，杆上存在与拉力平衡的**持续残差**，量级 ≈ **0.2 × 目标越界距离**（Session 2 实测：名义越界 50/150/400px → 稳态 maxError 26/36/89px）。渲染上表现为硬拽出界时的弹性「拉扯感」——原型手感已验证其自然；若日后不想要，下面的 settle 备选可把渲染态压回刚性。测试容差区分「拖拽中」与「静置后」两种状态，且拖拽态断言只在**指针静止后的稳态帧**上取样——快速甩动的瞬态帧残差没有稳定上界，对它断言测试会闪断（§8.2）。
...
| 6b | 三角板被捏翻 180°（镜像） | 距离约束手性盲：{BP,CP,BC} 对给定 B、C 有两个镜像解；大步软目标注入把 P 推过 BC 线，投影收敛到镜像支（非法物理，数值隧穿） | **Session 3 决策**，推荐 B+A：注入限步（clamp 单遍注入 < 板高，压制误翻）+ 接受故意暴力翻转为教学彩蛋；符号面积约束（选项 C）超范围，仅当升格为教学内容才考虑 | 复现：pointermove 跨线目标一帧即翻（Session 2 实测）；若做 B：限步后同样操作不翻 + 可达目标拖拽手感不回归 |
...
| 不可达拖拽 | 目标 (10⁴, 380)：全程无 NaN；松手后逐帧迭代恢复 <0.5px，断言 ≤8 帧（实测 4 帧） |
| 拖拽中残差分级 | 稳态帧取样：`maxError < 0.6×越界距离`（实测 ≈0.2×：越界 50/150/400px → 26/36/89px）；静置 <0.5px |
