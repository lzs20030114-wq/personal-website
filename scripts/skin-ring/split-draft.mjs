...
// 缓冲要跨过「自由段轴向跨度 − 箱高」：4·BUF ≥ 1.2·(M+BUF) − H
// 自由段 free = 2(M+BUF)+1 ≤ F_TOT，联立消去 BUF：
//     **M ≤ (1.4·F_TOT + H − 2) / 4**
// 满裂（缝深 = 挑出 D）时 M = D + H/4（**与缝宽无关**——缝宽只在台高与缝之间分配），
// 代入得本族的天花板：
//     **满裂平台的挑出 D ≤ 0.35·F_TOT − 0.5**   （F=133 ⇒ D ≤ 46.0px）
//
...
import {
  SQUARE, SQUARE_RUNGS, buildSquareOrder, squareAngle, squareBuffer, squareFree,
...
/** 材料账天花板：M 的上限（解析） */
const mCap = (fTot = F_TOT, e = SQUARE.E_MIN) => (1.4 * fTot + H - 2 - 1.4 * e) / 4;
/** 满裂平台挑出的上限（解析） */
...
function fitTier(Dwant, t, log = () => {}, opt = {}) {
  const cands = [];
  const lo = Math.max(20, Math.round((Dwant - PUFF - 6) / 2) * 2);
  for (let boxD = lo; boxD <= lo + 14; boxD += 2) {
    let lv;
...
if (MODE === 'audit') {
  ...
  console.log(`\n══ 材料账 ══\nM = 挑出/2 + 台高/2 + 缝深/2 + 缝尖宽/4   ·   M ≤ (1.4·F + H − 2 − 1.4·e)/4 = ${mCap().toFixed(1)}`);
  console.log(`满裂 ⇒ M = D + H/4 ⇒ D ≤ ${dCap().toFixed(1)}px（与缝宽无关）`);
  console.log(`若用 Lab.12 的 107：D ≤ ${dCap(107).toFixed(1)}px；若榨到 157（lead 8/tail 5）：D ≤ ${dCap(157).toFixed(1)}px`);
  console.log('\n══ 三档标定 ══');
  ...
  console.log('\n档   实测挑出   箱高    缝宽   自由段  缓冲   锁定       结  Δ     缝心离下缘  上缘   下缘');
  for (const r of out) {
    const m = r.m;
    console.log(
      `${r.nm}档  ${m.reach.toFixed(1).padStart(7)}  ${m.height.toFixed(1).padStart(5)}  ${m.mouth.toFixed(1).padStart(5)}  ` +
  ...
  console.log(`\n箱高散布 ${sp((r) => r.m.height)}px（目标 ${H}）· 缝心散布 ${sp((r) => r.m.align)}px · 上缘散布 ${sp((r) => r.m.topY)}px · 下缘散布 ${sp((r) => r.m.botY)}px`);
  ...
  const flat = SQUARE_TIERS.map((ti) => {
    const lv = build(squareReachOf(ti.k), 0, { k: ti.k });
    return { nm: ti.name, k: ti.k, m: run(lv) };
  });
  for (const f of flat) console.log(`  平档 ${f.nm}档 k${f.k}: 挑出 ${f.m.reach.toFixed(1)} 高 ${f.m.height.toFixed(1)} 缝心离下缘 ${f.m.align.toFixed(1)}`);
...
for (const s of [
  ...
  `   单箱 36 高，捏到最后是两片 12 高的台夹一道 12 的缝。总高一圈恒定 ⇒ 上下缘齐平。实测三档箱高 ${RUNS.map((r) => r.m.height.toFixed(1)).join(' / ')}（散布 0.39px）、上缘散布 1.3 / 下缘 0.9px。`,
  ...
  `材料账（本轮推导 + 实测对上）：M = 挑出/2 + 台高/2 + 缝深/2 + 缝尖/4 ≤ (1.4F+H−2−1.4e)/4 ⇒ **满裂平台的挑出 ≤ ${dCap().toFixed(1)}px**，与缝宽无关。`,
  '⇒ **4 重是被材料强制的，不是编制选择**：挑得远与缝得深抢同一份材料，故缝只能在面档最深、角档为零 —— 恰好就是方形的 4 重对称。',
  '   （1 重镜像编制在全部 20 个相位里都会把 L7 押上角位、2 重最好也是 L4，材料账都不够 —— 所以「一圈一个来回」在这个形状上做不出来。）',
  `实测对位：平档三档缝心离下缘全是 100.4，捏分三档 ${RUNS.slice().reverse().map((r) => r.m.align.toFixed(1)).join(' / ')} —— **六条一起散布 2.31px ⇒ 切编制时平台不跳**。`,
  '已知瑕疵（如实报）：成形中段三档的平台高度散布最大 14.9px（step 550，面档滞后），到 step 1000 收敛到 2.3px。Lab.12 v4 同口径峰值是 5.5px。',
  '   这属于**成形过程**的账，按 §16.4 是拍板通过后单独一轮的活（Lab.12 当初也是先定终态形、再做成形设计），不影响终态。',
...
Y += 8;
{
  ...
  ty += 20;
  for (const s of [
    '为什么小：满裂那一档的挑出被材料账封在 46px，方形由它反推。',
...
for (const s of [
  ...
  '④ **角档要不要留一道浅折痕**：材料上还剩余量（可给 ~20px 的凹槽）；现取「与平档同一份构造、只是浅一档的实心箱」。',
