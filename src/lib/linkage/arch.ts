 ...
 * 只是 LinkageDef 换成从最初 伸缩外壳1.3dm 的 S4 截面提取的同源真机几何：
 *   14 块角化三角板（刚性三角 = 3 杆，同「耦合三角板」原理）
 *   + 曲柄滑块驱动链（轮心锚点 → 曲柄销 → 中央杆 → 拱顶）
 *   + 竖直导轨 / 四个水平滑脚 / 两条向心轨道（见 arch-data.ts 头注）。
 ...
 * 同等刚性感需要更高的遍数预算。四滑脚版本 91 杆 × 64 遍 × 60fps ≈ 35 万投影/秒。
 * 死点附近的峰值与分支连续性由测试锁定，不能用固定外脚改变机构自由度。
 */
...
export function apexHeightMM(s: LinkageSolver): number {
  return (ARCH_GROUND_Y - s.nodes[ARCH_APEX].y) / ARCH_SCALE;
}
