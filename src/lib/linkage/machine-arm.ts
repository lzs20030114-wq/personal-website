 ...
 * 不要直挺挺伸着」）。
 *
 * 三腱在截面上相隔 120°，故让收缩量按 120° 相位差走余弦、**负值截零**，
 * 合成的弯向就是一个缓慢回转的向量——臂梢画圈，读作「摆动」。
 * 截零不是凑数：肌腱只能拉不能推，负收缩没有物理意义；截零后任意时刻
 * 至少有一根完全松弛（三个相隔 120° 的余弦恒和为零，不可能同时为正）。
 *
 * 再叠一层更慢的整体幅度调制 = 「舒卷」：整条臂缓缓卷紧又松开。
 * 两个周期取不可约的值（≈15s 与 ≈27s），合起来长时间不重复，免得看出循环。
 *
 * 幅度刻意小（≤0.22）——要的是有生气，不是表演卷曲；真要卷用滑块。
 * 全是手感常量，待用户真机拍板。
 */
export const ARM_IDLE = {
   ...
   */
  base: 0.26,
  /** 差动幅度：叠在基线之上，峰值收缩 = base + span = 0.42（远低于 J 形卷曲的 0.5） */
  span: 0.16,
  /** 弯向回转角速度 rad/s（2π/0.42 ≈ 15s 一圈） */
  sway: 0.42,
  /** 整体舒卷角速度 rad/s（2π/0.23 ≈ 27s 一轮） */
  curl: 0.23,
  /** 舒卷下限：差动幅度在 [floor, 1] × span 之间起伏 */
  floor: 0.55,
} as const;
 ...
 *
 * 实测效果（跑完整波形、量梢端相对基座的侧向偏移）：**38.8–87.1mm**，
 * 臂长 358mm 的 11%–24%——始终带弧、从不回到笔直，又远不到 J 形卷曲（c=0.5 → 128.5mm）。
 * 全是手感常量，待用户真机拍板。
 */
