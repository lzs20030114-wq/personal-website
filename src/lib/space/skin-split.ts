 ...
 *
 * ## 单级成形顺滑（2026-08-30 第二轮，用户「还是不够顺滑：抽一个单独做」）
 *
 * 在 L5 上逐步量位移破的案：级联只解决了「十级什么时候各自成形」，没解决
 * 「一级自己的成形是一声啪」——基线蠕动 0.30 px/步，而 step 594 一步之内
 * 外箱十颗键**同一步全锁**（吸引护栏早把全链备到 rb 附近，拉链只等最外键
 * 入窗；闸一开 55 迭代放行 55 颗），全部整形机制以锁定为开关，整个成形压缩
 * 在 4 步（0.05s）里，峰值 34.5 px/步 = 基线的 114 倍。帧上就是坯团一步变
 * 方箱。另有两类次级猛动：无锁定的屈曲翻越（缝壁材料抢位，23 px/步）与
 * 单键锁定触发的找平/硬投影（L3 实测 29.6 px/步）。
 *
 * 定案 = 两个引擎加法式选项的组合（SPLIT_GAP / SPLIT_CLAMP）：
 * - **lockGap 24，只圈外箱链**（lockGapChains:[0]）——同链两次锁定至少隔
 *   24 步，十颗键从一声啪摊成 ~220 步的逐挡拉链。**不能碰缝链**：深缝级靠
 *   缝链预锁成刚性手风琴（§15.8），给它上节拍 L8 直接绞死（Δ5.54→20.6、
 *   49 节环）。
 * - **stepClamp 4px/步**——机制无关的限速总闸，兜住节拍管不到的屈曲翻越与
 *   单键顿挫。**裸用会坏形**：限速 3px 单用时缝料赶不上箱体合拢的竞速，
 *   被关在箱里绞成环（L5 Δ3.29→9.88，三档限速同坏 ⇒ 是路径问题不是速度
 *   问题）；节拍先把合拢摊开 220 步，竞速窗口变宽，限速才安全。4px 是
 *   实测阈值：3px 仍会把缝壁重排拖出 18 节瞬态环，4px 起干净。
 * - gap 取 24 不取 20：L3 在 gap=20 时第 9 颗键恰撞上缝底重排的相位，冒
 *   9 节瞬态环；24 或 16 都避开（环 2）——环跟节拍相位有关，与限速无关。
 *
 * 十级验收（gap24+clamp4，密采样）：剪影Δ 与锁定集合**逐位复原**
 * （0.29–5.54），过程峰值全部恰好 4.0 px/步（= 既有成形期抖动的量级），
 * 最大瞬态环 ≤4（守门线 6），末锁 783–942 全部落在协议纪律解除（950）之前。
 */
/** 外箱拉链节拍（协议步；只作用于链 0 = 外箱梯） */
export const SPLIT_GAP = 24;
/** 每步位移上限（世界单位 = 4px；成形猛动的机制无关总闸） */
export const SPLIT_CLAMP = 0.04;
...
/** 站方公共选项 + 本族专属（缝链的作用域拆分 + 成形顺滑的节拍/限速） */
