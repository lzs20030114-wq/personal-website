 ...
 *
 * ## 对位：居中对齐（v4，构造见下方 SPLIT_TAIL 一节）
 *
 * 十级的外箱半跨 M 差得很远（24 → 49），折叠体若各自随谱漂，一排看过去就是台阶。
 * 现构造把配平垫劈成两半夹在结构两侧，使**缝心恒在两贴合锚的中点**、
 * 在每一个 r 上逐级相等；自由段仍取各级调好的自然长度（两端缓冲恒 4 节 =
 * 交接件纪律下限）。演进：v3「固定尾段 + 顶端配平」（底边齐平）→ v4 居中，
 * 两版的账与被否的两条弯路（全员同 FREE 居中 / 按终态芯长配平）见 §15.11、§15.14。
 *
...
/**
 * 对位构造 v4：**居中对齐**（用户 2026-08-30「所有形状都居中对齐，按照我们
 * 之前的做法」——之前的做法 = Lab.08 嘴心解析构造：不标定、不逐级凑数，
 * 把对齐写成构造性质、全程成立）。
 *
 * 七段谱 `[杆帽 8 | 上垫 | 隔离 16 | 结构 | 隔离 16 | 下垫 | 尾 53]`：
 * **配平垫劈成相等的两半放在结构两侧**（free 恒为奇数 ⇒ 107−free 恒为偶数
 * ⇒ 两半是精确整数，零取整误差），各隔一段 16 节贴合（= 全局约束最大跨距，
 * 双结构带 mid≥16 结论第四次复用 ⇒ 垫与结构互不相扰，逐级定案的动力学
 * 环境一字不变）。
 *
 * 为什么这就居中了：结构两端的锚都是**贴合段**（钉在芯上，位置由芯布局
 * 解析确定），对称折叠体的缝心恒在两锚中点 ⇒
 *   缝心离下缘 = (尾 + 隔离)·2px + (下垫 + 缓冲 + 半结构)·2r px
 *             = 138 + 114·r px
 * ——右边没有任何逐级量（下垫按 (107−free)/2 配平，恰好消掉 free），
 * 在**每一个 r 上**十级逐位相等；折叠状态也不进账（缝心 = 两钉点中点，
 * 由对称性）。残差只剩折叠体自身的上下不对称（Lab.08 实测 ±0.6px 量级）。
 * 带子上下缘照旧齐平（贴合总量与自由总量仍全员同值）。
 *
 * 代价（有意的）：底边不再齐平——两台对称地上下分开（顶边升 ~缝/2、
 * 底边降 ~缝/2），这正是「居中」的读法；v3 的「底边齐平」让位。
 * v3（配平垫全在顶端、结构坐在尾段上）的账与教训保留在 §15.11。
 */
export const SPLIT_TAIL = 53;
...
/** 结构自由段外裹上对位构造 v4：杆帽 + 半垫 + 隔离 + 结构 + 隔离 + 半垫 + 尾段 */
function wrap(seg: SkinSpec[number] & readonly ['f', number, ...unknown[]]): { spec: SkinSpec; base: number } {
  ...
  if (pad % 2 !== 0) throw new Error(`配平垫 ${pad} 非偶数，劈不成两半（free 应恒为奇数）`);
  const half = pad / 2;
  return {
    spec: [
      ['g', SPLIT_LEAD_A],
    ...
    base: SPLIT_LEAD_A + half + SPLIT_LEAD_B, // 结构段的绝对起点（marks 用）
  };
}
...
function levelNotched(i: number, tune: SplitTune): SkinSplitLevel {
  ...
  return {
    ...
    // 与 L0 及全站其余台架同窗口（±1 节 = ±2px，缝宽 10–28px 抹不掉）。
    // 旧值 [2,1] 是个误解：偶数窗口并没有变窄（half=w>>1 仍是 ±1），
    // 只是把 renderSmooth 的归一化撞出 1.5× 放大——九个刻缝级因此比 L0
    // 大了一圈（2026-08-30 用户「最左边的没同步」查出，引擎侧已修）。
    smooth: [3, 1],
