 ...
 *
 * 反解位置（给定目标嘴心 Y*）：bT/(bT+bB) = (Y* − M/2 − 2·lead) / (S − M)
 * 两个整数旋钮把它落到格点上：**lead**（贴合段节数，2px/节，严格线性，纯平移）
 * 与 **扇形在自由段内的位置 dFan**（上下缓冲此消彼长，实测 ~1.9px/节；f 不变 ⇒
 * 折叠体的形状与高度逐位不变，实测高度差 0.00、键数不变）。
 * 标定脚本 scripts/skin-array/calibrate.mjs：网格实跑取整数最优，
 * 嘴心散布 3.38 → **1.01px**。改任何形状后须重标（守门卡散布）。
 *
...
/**
 * 位置标定表（scripts/skin-array/calibrate.mjs 实跑网格取整数最优，目标嘴心 127.9px）。
 * LEAD = 贴合段节数（2px/节）；DFAN = 扇形在自由段内的位置（~1.9px/节，f 不变 ⇒ 形状不变）；
 * TAIL 配平使各带总长恒定（帘子下缘齐；尾段是贴合段、钉轴、不对折叠体施力）。
 */
const LEAD = [54, 54, 54, 54, 54, 56, 54, 55, 55, 56, 55, 55];
const DFAN = [2, 2, 1, 1, 1, -1, 1, 0, 0, -1, 0, -1];
const TAIL = [56, 56, 55, 55, 55, 52, 54, 52, 52, 51, 51, 51];
