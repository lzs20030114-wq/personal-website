 ...
 *
 * **lead = 对位常数 54（2026-08-20 用户第二轮纠偏）**：首版对位用整体渲染纵移，
 * 被否——「最上面接天花板的部分不就也不对齐了吗？改变形状在线上的位置来做到
 * 折叠后的形状居中对齐」。lead（贴合段节数）正是「形状在带子上的位置」：它是
 * 纯平移参数（重力均匀、约束全是相对量 ⇒ 折叠形状对它严格不变），每节挪
 * SEG·100 = 2px 且与收缩进度无关（贴合段以未收缩间距钉轴——从 coreLen =
 * 贴合节数·SEG + 键谱段·SEG·r 反推证实）。标定（scratch calib_lead）：全员
 * 取 54 即把折叠中线散布 14.79px 收到 1.22px（< 1 节粒度 2px），顶端天然全部
 * 贴天花。原「lead 手排错峰」时间表随之退役——位置分量被对位归零，相邻形态
 * 距离只剩形状本身的差异，比错峰更平滑。
 *
...
/** 对位常数（见文件头「lead = 对位常数 54」；改任何形状后需重标——守门测试卡终态中线散布） */
export const ARRAY_LEAD = 54;
...
export function buildTransitionArray(): SkinArrayUnit[] {
  ...
  for (let i = 0; i < n; i++) {
    const t = i / (n - 1);
    if (i === 0 || i === n - 1) {
      // 端点：键谱段与尾段 = 原谱对象逐字复用（形态即用户逐轮磨定的那两台）；
      // 只有 lead 换成对位常数——它是位置不是形状
      const d = i === 0 ? A : B;
      const spec: SkinSpec = [['g', ARRAY_LEAD], d.spec[1], d.spec[2]];
      out.push({
        t, spec,
        opts: skinSiteOpts(d),
    ...
    const spec: SkinSpec = [['g', ARRAY_LEAD], seg, ['g', 50]];
