...
export const SQUARE = {
   ...
   *
   * **2026-09-02 第二次加长 250 → 305**：捏分编制从四次循环改成一次循环（用户拍板
   * 「一次循环优先，厚度让路」）。一次循环要面类在**同一个挑出**上既做出整块又做出满裂，
   * 满裂那一档的挑出上限随带长走（材料账封顶）；实测 250 上一次循环在任何箱高都配不出来，
   * 305 上箱高 44 配得出。平档 / 起伏 / 圆↔方五档形与平台高度仍逐位不变（lead 70 → 125）。
   */
  BAND: 305,
...
export const SQUARE_WAVE = {
   ...
   * **2026-09-02 第二次随带长 250 → 305 再上移 55**（115…135，中心 125 = 平档 lead）。
   */
  LOW: 135,
  /** 波峰（lead 小 = 高） */
  HIGH: 115,
